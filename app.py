...
modelrf = joblib.load("D:/CLG/Sem5/DWM/DWMPROJECT/DiseasePrediction/saved_model/random_f.joblib")
modeldt = joblib.load("D:/CLG/Sem5/DWM/DWMPROJECT/DiseasePrediction/saved_model/decision_tree.joblib")
...
st.title("Malady Prediction Using Symptoms")
# st.header("This is the alpha testing phase stay tuned for further progress 🩺")
# st.write("You'll be able to select symptoms and with a click our backend will run predictions using Data Mining Algorithms")
...
predictionrf = modelrf.predict(query)
predictiondt = modeldt.predict(query)
...
if st.button("Evaluate"):
    with st.spinner('Predicting output...'):
        ...
        if symptoms:
            ...
            st.write("We Predict it  is: ")
            st.error(*predictionrf)
            st.write("By Using Random Forest Data Mining Algorithm")
            st.error(*predictiondt)
            st.write("By Using Decision Tree Data Mining Algorithm")
